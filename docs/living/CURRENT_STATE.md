...
- Clustering analyze dispatch is registry-driven (`src/study_query_llm/pipeline/clustering/registry.py`) for built-ins: `hdbscan+fixed`, `kmeans+normalize+pca+sweep`, `gmm+normalize+pca+sweep`, and `agglomerative+fixed-k`. Runners emit `cluster_labels`, `summary_metrics`, and `recipe_hash` per the Bundled Clustering Subsystem output schema.
