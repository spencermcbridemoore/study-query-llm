...
# Repo root: .../study-query-llm (from src/study_query_llm/experiments/runtime_sweeps.py)
...
async def main_bigrun_300_sweep(
    force: bool = False,
    ...
    if run_ids_ingested and not request_id:
        ...
        try:
            with db.session_scope() as session:
                ...
                linked = 0
                for pos, rid in enumerate(run_ids_ingested):
                    existing_link = (
                    ...
                    if not existing_link:
                        provenance.link_run_to_clustering_sweep(sweep_id, rid, position=pos)
                        linked += 1
...
def main_bigrun_sync(argv: Optional[list[str]] = None) -> None:
    """CLI sync entry: parse argv and run async main.

    When ``argv`` is None, uses ``sys.argv[1:]`` (standalone script invocation).
    """
    if argv is None:
        argv = sys.argv[1:]
