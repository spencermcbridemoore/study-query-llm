"""Grammar-bound fixed-k KMeans runners (kmeans + spherical-kmeans families)."""
...
def _resolve_k(parameters: Mapping[str, Any], *, n_samples: int) -> int:
    ...
    if raw is None:
        raise ValueError("fixed-k KMeans requires integer parameter k")
    k = int(raw)
    if k < 2:
        raise ValueError(f"fixed-k KMeans requires k >= 2, got {k}")
    if k >= n_samples:
        raise ValueError(
            f"fixed-k KMeans requires k < n_samples (k={k}, n_samples={n_samples})"
        )
    return k


@allow_no_run_stage
def run_kmeans_fixed_k_analysis(
    *,
    method_name: str,
    input_group_id: int,
    ...
    """Run sklearn KMeans with fixed k; grammar-bound preprocessing chain."""
    ...
    init = str(parameters.get("init", "k-means++"))
    ...
    summary_base = "spherical-kmeans" if base_algorithm == "spherical-kmeans" else "kmeans"
    summary = {
        "method_name": str(method_name),
        "base_algorithm": summary_base,
        ...
        "objective": {"inertia": float(model.inertia_)},
    }
    labels_payload = {
