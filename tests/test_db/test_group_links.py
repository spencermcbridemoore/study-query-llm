...
def test_get_group_children(db_connection):
    ...
    with db_connection.session_scope() as session:
        ...
        step1_id = repo.create_group(group_type="step", name="step_1")
        step2_id = repo.create_group(group_type="step", name="step_2")
        step3_id = repo.create_group(group_type="step", name="step_3")

        # Create links
        repo.create_group_link(run_id, step1_id, "step", position=1)
        repo.create_group_link(run_id, step2_id, "step", position=2)
        repo.create_group_link(run_id, step3_id, "step", position=3)

        # Get children
        children = repo.get_group_children(run_id, link_type="step")
...
def test_get_group_parents(db_connection):
    ...
    with db_connection.session_scope() as session:
        ...
        parent_id = repo.create_group(group_type="run", name="test_run")
        child_id = repo.create_group(group_type="step", name="step_1")

        # Create link
        repo.create_group_link(parent_id, child_id, "step", position=1)
...
def test_group_link_different_types(db_connection):
    ...
    with db_connection.session_scope() as session:
        ...
        run_id = repo.create_group(group_type="run", name="test_run")
        step_id = repo.create_group(group_type="step", name="step_1")

        # Create different link types
        repo.create_group_link(embedding_batch_id, run_id, "generates")
        repo.create_group_link(run_id, step_id, "step", position=1)
        ...
        steps = repo.get_group_children(run_id, link_type="step")
        assert len(steps) == 1
...
def test_create_group_link_duplicate(db_connection):
    ...
    with db_connection.session_scope() as session:
        ...
        link_id1 = repo.create_group_link(
            parent_id, child_id, "step", position=1
        )
