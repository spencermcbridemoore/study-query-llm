#!/usr/bin/env python3
"""Request-mode worker for local 300 (2 datasets, 14 embedders, 4 summarizers).

This worker executes only missing runs for a clustering_sweep_request and uses
claim/lease semantics to avoid duplicate work across concurrent workers.
"""
...
def _missing_triples_for_request(request_id: int):
    with db.session_scope() as session:
        repo = RawCallRepository(session)
    ...
    for rk in missing_run_keys:
        ...
        if t:
            triples.append((rk, t["dataset"], t["embedding_engine"], t["summarizer"]))
...
def run_worker(
    ...
    while True:
        run_rows, progress = _missing_triples_for_request(request_id)
        if not run_rows:
        ...
        by_engine = {}
        for row in run_rows:
            by_engine.setdefault(row[2], []).append(row)
        ...
        for current_engine in engines:
            ...
            emb_provider = provider_cache[current_engine]
            for run_key, dataset_name, _, summarizer_key in rows:
        ...
        if max_runs is not None and done >= max_runs:
            break
        if embedding_engine:
            refreshed_rows, _ = _missing_triples_for_request(request_id)
            engine_missing = [r for r in refreshed_rows if r[2] == embedding_engine]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Worker for local_300_2datasets request-mode sweep")
    ...
    worker_id = args.worker_id or f"local300-worker-{os.getpid()}"
