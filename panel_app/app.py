    ...
    def update_embeddings():
        ...
        try:
            ...
            with db.session_scope() as session:
                ...
                # Update deployment filter options
                deployments = set()
                ...
                # Get cache hit info from metadata
                cache_hits = 0
                cache_misses = 0
                total_latency = 0
                latency_count = 0
                ...
                # Get dimension info from EmbeddingVector
...
"""
                ...
                if embedding_calls:
                    embeddings_data = []
                    for call in embedding_calls[:100]:  # Limit to 100 for display
                        # Extract input text from request_json
                        request = call.request_json or {}
                        input_text = ''
                        if isinstance(request, dict):
                            input_text = request.get('input', '') or request.get('text', '') or ''
                        
                        # Get embedding vector info
                ...
                # Show embedding vectors for successful calls
                if call_ids:
                    vectors = session.query(EmbeddingVector).filter(
                        EmbeddingVector.call_id.in_(call_ids[:50])  # Limit to 50
                    ).all()
