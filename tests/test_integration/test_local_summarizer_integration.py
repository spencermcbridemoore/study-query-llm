...
@pytest.mark.asyncio
async def test_chat_provider_roundtrip(ollama_available):
...
@pytest.mark.requires_ollama
@pytest.mark.asyncio
async def test_chat_provider_respects_max_tokens(ollama_available):
    """Setting max_tokens=10 should produce a short response."""
    provider = OpenAICompatibleChatProvider(
        base_url=OLLAMA_URL, model=TEST_MODEL
    )
    result = await provider.complete(
...
@pytest.mark.requires_ollama
@pytest.mark.asyncio
async def test_summarization_service_local_llm(ollama_available):
...
@pytest.mark.requires_ollama
def test_create_paraphraser_local_llm_sync(ollama_available):
