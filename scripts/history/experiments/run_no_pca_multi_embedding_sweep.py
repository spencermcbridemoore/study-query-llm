...
from scripts.history.experiments.run_experimental_sweep import LLM_SUMMARIZERS
...
EMBEDDING_CACHE_DIR = os.environ.get("EMBEDDING_CACHE_DIR") or str(REPO_ROOT / "data" / "embedding_cache")
    ...
    for dataset_name, dataset_info in loaded_datasets.items():
        ...
        for engine_entry in EMBEDDING_ENGINES:
            ...
            for summarizer_entry in LLM_SUMMARIZERS:
                ...
                run_key = f"{dataset_name}_{engine_safe}_{sum_safe}_{ENTRY_MAX}_labelmax{label_max}_multi"
                if run_key_exists_in_db(db, run_key):
                    print(f"\n      [{run_count}/{total_runs}] {summarizer_name} (SKIP - run_key in DB)")
                    continue
                ...
                with ollama_vram_scope(llm_deployment, llm_provider):
                    ...
                    try:
                        run_id = ingest_result_to_db(
                            result, metadata, filtered_labels, db, run_key
                        )
