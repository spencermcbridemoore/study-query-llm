...
- All-variant clustering analysis backfill (registry-driven): `study_query_llm.experiments.clustering_analysis_backfill` expands every `iter_algorithm_specs()` entry into deterministic `(snapshot, embedding_batch, method, parameters)` targets with `analysis_run_key` prefix `backfill_exec__`, validates parameter payloads against each spec `parameters_schema`, optionally persists resume state JSON, and caps catastrophic failure rates when executing `pipeline.analyze.analyze`. Snapshot->embedding_batch matching keys on `(source_dataframe_group_id, source_dataframe_row_count)` against `embedding_batch.entry_max` (the full-dataframe embed row count), because `analyze` consumes the full embedding matrix sliced by the snapshot resolved-index. The single-engine operator entrypoint is `scripts/living/backfill_all_variant_clustering_analysis.py` (writes manifests under `experimental_results/backfill_manifests/`). The prior strict per-pair orchestration surface (`run_per_pair_backfill_orchestrator.py` + `run_per_pair_backfill_worker.py` + the `BACKFILL_CLUSTERING_PER_PAIR.md` runbook) was retired as a spent campaign whose acceptance step depended on a since-deleted `scratch/` verifier; see ledger C081.
