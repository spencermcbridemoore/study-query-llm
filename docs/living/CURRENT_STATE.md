...
- All-variant clustering analysis backfill (registry-driven): `study_query_llm.experiments.clustering_analysis_backfill` expands every `iter_algorithm_specs()` entry into deterministic `(snapshot, embedding_batch, method, parameters)` targets with `analysis_run_key` prefix `backfill_exec__`, validates parameter payloads against each spec `parameters_schema`, optionally persists resume state JSON, and caps catastrophic failure rates when executing `pipeline.analyze.analyze`. Operator entrypoint: `scripts/living/backfill_all_variant_clustering_analysis.py` (writes manifests under `experimental_results/backfill_manifests/`); optional `--snapshot-ids-file` loads large shard lists; `scripts/living/run_two_engine_backfill_sharded.py` chains preflight → round-robin sharding (default 8) → parallel child processes per engine → global verify + `ACCEPTANCE_SUMMARY.md` (defaults: `text-embedding-3-large`, `embed-v-4-0`). Optional `--snapshot-ids-file` on the sharded runner scopes **preflight** and **verify** manifests to a fixed snapshot subset (execute already shards via per-worker snapshot JSON). Multi-pair overnight sequencing for snapshots **6/9/10/21/22**: `scripts/living/run_overnight_fivesnap_backfill.py` (`init` → `preflight` → timeboxed `execute` → `verify-package`). Runbook: `docs/runbooks/BACKFILL_CLUSTERING_TWO_ENGINE_SHARDED.md`.
