...
Run label_pre_fix_runs (root wrapper or this directory) first to mark the runs.

Prerequisites:
    docker compose --profile postgres up -d db
    python scripts/init_local_db.py  (first time only)
    python scripts/label_pre_fix_runs.py  (label runs before archiving; or
    python scripts/history/sweep_recovery/label_pre_fix_runs.py)

Usage:
    python scripts/archive_pre_fix_runs.py --dry-run   # preview, no changes (root wrapper)
    python scripts/history/sweep_recovery/archive_pre_fix_runs.py --dry-run
