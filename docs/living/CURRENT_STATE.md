...
- The `analysis_request` get-or-create identity `(method_name, input_id, run_key)` is guarded by the partial UNIQUE index `uq_groups_analysis_request_identity` (NULL-distinct; the dual-dialect contract is single-sourced in `src/study_query_llm/db/analysis_request_identity.py`). Verified applied and clean on canonical (0 duplicates, 0 half-rows) on 2026-06-04; see [`docs/review/CANONICAL_INVENTORY_2026-06-04.md`](../review/CANONICAL_INVENTORY_2026-06-04.md).
- Unified execution-provenance entity `ProvenancedRun` now uses canonical `run_kind=execution` for new writes; semantic role is stored in `metadata_json.execution_role` (`method_execution` / `analysis_execution`), with method-stage semantics stored separately in `metadata_json.pipeline_stage_role` (`source` / `imported` / `subset` / `export`) and optional `pipeline_stage_context`.
...
- `OpenAICompatibleChatProvider` now forwards provider-specific chat kwargs (for example `logprobs`, `top_logprobs`, `seed`) through `chat.completions.create(...)`; reserved envelope keys remain guarded.
...
- Request fulfillment/progress semantics are request-scoped: completed runs are counted from request delivery links (`request -> contains -> run`) rather than global `run_key` existence.
- Optional single-writer request finalization path is available behind `SQ_USE_REQUEST_FINALIZER_JOB`: planner emits one request-level `finalize_request` node, per-run `finalize_run` jobs persist run facts + delivery links, and request fulfillment/sweep linkage is performed by the single request-level finalizer.
...
- Job execution dispatch is registry-based in `job_runner_factory.py` (including `langgraph_run` and optional clustering `finalize_request`), and reduce/finalize execution uses a typed reducer plugin seam that wraps the existing clustering reducer implementation.
- Register-first retirement is active for prior lazy-registration drift points:
  - `pipeline.analyze._resolve_method_definition_id` now fails loud for missing non-composite method rows.
  - `SweepRequestService.record_analysis_result` now fails loud when analysis methods are not pre-registered.
  - `langgraph_provenance.record_langgraph_job_outcome` now requires pre-registered methods (no lazy inserts).
...
- `embed` matrix writes now include matrix-level lease single-flight (`EmbeddingCacheLease`) keyed by dataset/engine/provider/entry_max/key_version, so concurrent same-key runs reuse a single winner result instead of creating duplicate same-key `embedding_batch` groups.
...
- All-variant clustering analysis backfill (registry-driven): `study_query_llm.experiments.clustering_analysis_backfill` expands every `iter_algorithm_specs()` entry into deterministic `(snapshot, embedding_batch, method, parameters)` targets with `analysis_run_key` prefix `backfill_exec__`, validates parameter payloads against each spec `parameters_schema`, optionally persists resume state JSON, and caps catastrophic failure rates when executing `pipeline.analyze.analyze`. Single-engine operator entrypoint remains `scripts/living/backfill_all_variant_clustering_analysis.py` (writes manifests under `experimental_results/backfill_manifests/`). The active multi-pair operator surface is now strict per-pair orchestration: `scripts/living/run_per_pair_backfill_orchestrator.py` + `scripts/living/run_per_pair_backfill_worker.py`, with one worker process per `(snapshot_id, embedding_engine)` pair, UNION pair inventory over snapshots `6/9/10/17/18`, phase0 embed precompute for `no_embedding_batch`, phase1 clustering execution, bounded retries, and infeasible-modulo acceptance artifacts (`orchestrator_summary.json`, `infeasible_pairs.json`, `verification_summary.json`, `ACCEPTANCE_SUMMARY.md`) under `experimental_results/backfill_per_pair/<utc>/`. Runbook: `docs/runbooks/BACKFILL_CLUSTERING_PER_PAIR.md`.
- Sweep-and-select clustering curves (per-candidate `k` metrics for registry `fit_mode=sweep_select` methods): `study_query_llm.experiments.selection_curve_export` reads `analysis_results` artifact URI maps and expands each ``*_selection_curve.json`` into CSV rows; operator CLI `scripts/living/export_sweep_selection_curve_metrics_csv.py`.
- Sharded sweep reducer retains full leaf try provenance: `JobReducerService.reduce_k_job` attaches sorted `by_k[*].tries` records while preserving legacy `objectives` / `labels_all`; `experiments.sweep_io.serialize_sweep_result` forwards `tries` into JSON blobs consumed by `ingest_result_to_db`.
