"""Grammar-bound agglomerative clustering with normalize/pca preprocessing chains."""
...
def _resolve_k(parameters: Mapping[str, Any], *, n_samples: int) -> int:
    ...
    if k >= n_samples:
        raise ValueError(
            f"agglomerative fixed-k requires k < n_samples "
...
@allow_no_run_stage
def run_agglomerative_preproc_fixed_k_analysis(
    *,
    method_name: str,
    input_group_id: int,
    ...
    """AgglomerativeClustering with grammar-bound preprocessing (normalize/pca)."""
